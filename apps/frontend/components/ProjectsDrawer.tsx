...
function useProjects() {
    ...
    useEffect(() => {
        (async () => {
           ...
           const projectsByDate = sortedProjects.reduce((accumulator: {[date: string]: Project[]}, project: Project) => {
                const date = new Date(project.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
                if (!accumulator[date]) {
                    accumulator[date] = [];
                }
                accumulator[date].push(project);
                return accumulator;
            }, {});
							...
							{projects[date]
								...
								.map((project) => (
									<Button
										...
										className="pl-2 py-3 w-full text-left justify-start items-start rounded hover:bg-accent cursor-pointer hover:text-accent-foreground text-muted-foreground wrap-break-word whitespace-normal line-clamp-2"
									>
										{(project.description)}
									</Button>
								))}
